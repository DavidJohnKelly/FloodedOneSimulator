...
public class DTNSimGUI extends DTNSimUI {
	...
	private void processAssertionError(AssertionError e) {
		...
		StringBuilder txt = new StringBuilder((msg != null ? msg : "") + " at simtime " +
                SimClock.getIntTime() + "\n\ncaught at:\n" +
		...
		int selection = JOptionPane.showOptionDialog(getParentFrame(), txt.toString(),
		...
		if (selection == 0) {
			...
			for (StackTraceElement trace : e.getStackTrace()) {
				txt.append(trace.toString()).append("\n");
			}
			JOptionPane.showMessageDialog(getParentFrame(), txt.toString(),
    ...
    /**
     * Updates play field and sim time field
     *
     */
    ...
    /**
     * Sets a node's graphical presentation in the center of the play field view
    ...
    public Coord getCenterViewCoord() {
    	...
    	midX = sp.getHorizontalScrollBar().getValue() + 
    		(double) sp.getViewport().getWidth() / 2;
    	midY = sp.getVerticalScrollBar().getValue() + 
    		(double) sp.getViewport().getHeight() / 2;

    	return this.field.getWorldPosition(new Coord(midX, midY));	    	
    }
    
    /**
     * Sets certain location to be in the center of the play field view
	...
	/**
	 * Handler for play field's mouse clicks.
	 */
